...
SoundFile sound;  //新加這一行 讀beat.mp3
PitchDetector pitchDetector;//宣告 PitchDetector物件變數
int i;

void setup() {
  size(640, 320);
  background(0);
  sound = new SoundFile(this,"beat.mp3");//新加這一行 讀beat.mp3
  sound.loop();
  pitchDetector = new PitchDetector(this);//新增 初始化PirchDetector 物件
  //pitchDetector.input(new AudioIn(this, 0));原本的程式是用 Audio In 音效卡的音源輸入
...
void draw() {
  ...
  if (pitchAndConfidence[1] > 0) {
    ...
    println(pitchAndConfidence[0],pitchAndConfidence[1]);//把pitch 和confidence都印出來
  }
